...
public class Main {
  public static void main(String[] args) {
    ...
    int total = 0;
    long totalTaken = 0, maxRuntime = 0;
    for (int i = 0; i < inputs.length; i++) {
      if (inputs[i].length > 1)
        System.out.println("     Test Batch #" + (i + 1) + ": ");
      total += score[i];
      ...
      for (int j = 0; j < inputs[i].length; j++) {
        ...
        try {
          ...
          System.out.println("IE  (Error: Output file " + outputs[i] + " not found)");
          return;
        }
        ...
        if (m.error) {
          ...
          if (fullFeedback)
            System.out.print("(" + m.errorMessage + ") ");
          System.out.printf(" [%.3f seconds]", (double) taken / 1000);
          if (inputs[i].length == 1)
            System.out.printf(" (0/%d points earned)", score[i]);
          System.out.println();
          failed = true;
          continue;
        }
        ...
        System.out.printf(" [%.3f seconds]", (double) taken / 1000);
        if (inputs[i].length == 1)
          System.out.printf(" (%d/%d points earned)", wrong ? 0 : score[i], score[i]);
        System.out.println();
        if (wrong && showOutput) {
          System.out.println("Your output (clipped): ");
          System.out.print(SourceCode.out.length() > 15 ? SourceCode.out.substring(0, 15) : SourceCode.out);
